...
class Press_Sync_Data_Synchronizer {
	 ...
	 *
	 * @param Press_Sync_Media_Handler $media_handler Media Handler class.
	 */
	 ...
	 * @param string $duplicate_action Type of duplication action (e.g., sync, skip).
	 * @param bool   $force_update     Whether to override existing content.
	 *
	 * @return array
	 */
	public function sync_post( $post_args, $duplicate_action, $force_update = false ) {
		...
		$post_parent_id = $post_args['post_parent'];

		// Check for post parent and update IDs accordingly. @TODO: I think we can refactor this.
		if ( isset( $post_args['post_parent'] ) && $post_parent_id ) {
		...
		if ( ! $local_post && 'sync' === $duplicate_action ) {
		...
		if ( ! $force_update
		     && $local_post
		     && ( strtotime( $local_post['post_modified'] ) >= strtotime( $post_args['post_modified'] ) )
		) {
	 ...
	 * Sync a user.
	 *
	 * @param array  $user_args        Array of arguments.
	 * @param string $duplicate_action Type of duplication action to take (e.g., sync, skip).
	 * @param bool   $force_update     Whether to overwrite the existing value.
	 */
	public function sync_user( $user_args, $duplicate_action, $force_update = false ) {
		...
		foreach ( $user_args['meta_input'] as $usermeta_key => $usermeta_value ) {
			update_user_meta( $user_id, $usermeta_key, $usermeta_value ); // @codingStandardsIgnoreLine
		}
	...
	public function sync_objects( $request ) {
		...
		$wpdb->query( 'SET AUTOCOMMIT = 0;' ); // @codingStandardsIgnoreLine
	 ...
	 * Sync an attachment.
	 *
	 * // @TODO Figure out how or whether to reuse insert_new_media method in the Media_Handler class.
	 *
	 * @param array  $attachment_args  Attachment details.
	 * @param string $duplicate_action Type of duplication action to take (e.g., sync, skip).
	 * @param bool   $force_update     Whether to overwrite the existing value.
	 *
	 * @return mixed
	...
	public function sync_attachment( $attachment_args, $duplicate_action, $force_update = false ) {
		...
		$media_id = $this->media_handler->media_exists( $attachment_url );
		...
		if ( is_wp_error( $temp_file ) ) {
			@unlink( $file_array['tmp_name'] ); // @codingStandardsIgnoreLine @TODO We probably shouldn't suppress errors.
	...
	/**
	 * Retrieve a post that has been synced.
	 *
	 * @param array $post_args Post details.
	 *
	 * @return array|bool
	 */
	public function get_synced_post( $post_args ) {
		...
		// @codingStandardsIgnoreStart @TODO Should we have a DB Query class?
		$sql = "
	...
	/**
	 * @param array $comment_args Comment details.
	 *
	 * @return array|bool
	 */
	public function comment_exists( $comment_args = array() ) {
		...
		return $comment ? (array) $comment[0] : false;
	}
	...
	public function get_post_by_orig_id( $press_sync_post_id ) {
		...
		// @TODO Should we have a DB Query class?
	...
	/**
	 * Attach comments to a post object.
	 *
	 * @param $post_id
	 * @param $comments
	 */
